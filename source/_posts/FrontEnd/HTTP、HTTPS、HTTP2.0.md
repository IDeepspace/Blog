...
但是这样会造成一些性能上的缺陷，**客户端和服务器端每次建立和关闭连接都是一个相对比较费时的过程，并且会严重影响客户机和服务器的性能**。



#### 3、HTTP 1.1

为了解决 `HTTP 1.0` 的缺陷，`HTTP 1.1` 支持持久连接 `Keep-Alive`（持久连接、连接重用），**在一个 `TCP` 连接上可以传送多个 `HTTP` 请求和响应**，减少了建立和关闭连接的消耗和延迟。

在 `HTTP1.1`  中，服务器返回消息，客户端可以继续发起下一个请求，如果没有要发送的了，客户端最后发送 `Connection: close` 首部给服务器，这样就会进行关闭。
...
<img src="https://github.com/IDeepspace/ImageHosting/raw/master/FrontEnd/what-is-a-tcp-3-way-handshake-process-three-way-handshaking-establishing-connection.jpg" style="zoom: 60%;" />
