...
重载（`overload` ，发生在一个类中，方法名必须相同，参数不同）就是**编译时多态**的一个例子，编译时多态在编译时就已经确定。
...
程序中定义的**引用变量所指向的具体类型**和**通过该引用变量发出的方法**调用在编程时并不确定，而是在程序运行期间才确定，**即一个引用变量到底会指向哪个类的实例对象，该引用变量发出的方法调用到底是哪个类中实现的方法，必须在由程序运行期间才能决定。**

这样，在程序运行时才确定具体的类，我们就不用修改源程序代码，就可以让引用变量绑定到各种不同的类实现上，从而让该引用调用的具体方法随之改变，让程序可以选择多个运行状态，这就是多态性。

**那怎么做到呢？**

我们可以这么做：子类 `Child` 继承父类 `Father`，编写一个指向子类的父类类型引用，该引用既可以处理父类 `Father` 对象，也可以处理子类 `Child` 对象，当调用方法时，该对象就会根据自己所属的引用而执行不同的行为。**即相同的消息使得不同的类做出不同的响应。**
...
- 在 `Cat` 类中，重载了`eat` 方法，重写了父类 `Animal` 的 `sleep` 方法。重载之后的 `eat(String name)` 方法和父类 `Animal` 的 `eat()` 方法不是同一个方法，因此，在向上转型时会**丢失**（因为 `Animal` 中没有）；
